...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        is_dropped = tf.constant(0, tf.float32, name="is_dropped")
        mask = tf.constant(0, tf.float32, name="mask")        
        invrted_drop_out = 0
        ...
        def residual(name, l, increase_dim=False, first=False):
            ...
            with tf.variable_scope(name) as scope:
                ...
                if self.is_training:
                    ...
                    rounded_is_dropped = tf.round(is_dropped)
                    add_moving_summary(rounded_is_dropped)
                else:
                ...
                l = short_cut + (1.0 - mask) * invrted_drop_out * l
                if self.is_training:
                    resBlock_Output = tf.reduce_sum(l)
                    add_moving_summary(resBlock_Output)
...
def get_config(out_dir):
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate', [(0, 1e-1), (82, 1e-2), (123, 1e-3), (300, 0.0002)]),
...
if __name__ == '__main__':
    ...
    if args.gpu:
        #assert tf.test.is_gpu_available(), "error4"
